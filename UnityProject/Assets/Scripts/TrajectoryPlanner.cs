...
public class TrajectoryPlanner : MonoBehaviour
{
	...
	Collider m_TargetCol;
	Vector3 m_TargetOriginalScale;
    ...
    void Start()
    {
		...
		m_TargetOriginalScale = m_Target.transform.localScale;
		
		if (!m_TargetRb || !m_TargetCol)
		{
		    Debug.LogError("Target must have Rigidbody and Collider.");
		}

		if (!m_Tool0)
		{
		    Debug.LogError("Tool0 Transform not assigned in Inspector.");
		}
    }
	...
	void AttachTarget()
	{
		...
		m_TargetRb.collisionDetectionMode = CollisionDetectionMode.Discrete;

		m_Target.transform.SetParent(m_Tool0, true); // KEEP WORLD TRANSFORM
		m_Target.transform.localScale = m_TargetOriginalScale;
	}
	...
	void DetachTarget()
	{
		m_Target.transform.SetParent(null, true);
		m_Target.transform.localScale = m_TargetOriginalScale;

		m_TargetRb.isKinematic = false;
		m_TargetRb.useGravity = true;

		m_TargetRb.velocity = Vector3.zero;
		m_TargetRb.angularVelocity = Vector3.zero;

		m_TargetRb.collisionDetectionMode = CollisionDetectionMode.ContinuousDynamic;
	}

    IEnumerator DetachTargetSafely()
	{
		// PHASE 1 — detach but keep kinematic
		m_Target.transform.SetParent(null, true);
		m_Target.transform.localScale = m_TargetOriginalScale;
		...
		// PHASE 2 — let articulation settle
		yield return new WaitForFixedUpdate();

		// PHASE 3 — enable physics
		m_TargetRb.isKinematic = false;
		m_TargetRb.useGravity = true;
		m_TargetRb.collisionDetectionMode = CollisionDetectionMode.ContinuousDynamic;
    ...
    IEnumerator ExecuteTrajectories(MoverServiceResponse response)
    {
        if (response.trajectories != null)
        {
            ...
            OpenGripper();
            yield return new WaitForSeconds(0.2f);
			StartCoroutine(DetachTargetSafely());	
        }
    }
